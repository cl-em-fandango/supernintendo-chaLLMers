...
mechanics are T09's, the ownership sidecar is T51's, and the automatic
...
is the handler surface:
  ...
  * the `claimed` status row — lifecycle row order, the `<id> (<age>h)` labels,
    ...
    provider cannot age listed without an age, and ownership sidecars never
...
# names whatever owner the sidecar records, so the ids only have to be
...
class StatusClaimRowTest(_WiredFixture):
    ...
    def test_ownership_sidecars_are_never_listed_as_claims(self):
        self._plant("009-stuck.md", owner=PEER, hours_old=STALE_HOURS)
        self.assertEqual(self._sidecar_names(), ["009-stuck.md.claim.json"])
        self.assertIn(_row(CLAIMED_LOCATION, 1, "009-stuck (48h)"),
                      self._status_rows())
        self.assertNotIn(".claim.json", self._logged())
...
class EmptyReclaimTest(_WiredFixture):
    ...
    def test_an_orphan_sidecar_is_not_a_claim(self):
        """A sidecar with no markdown under it is reported by the audit, not swept."""
        metadata_path(self.claimed / "004-d.md").write_text('{"owner": "x"}')
        self.assertEqual(handlers.cmd_requeue_claims(older_than=THRESHOLD), 0)
        self.assertIn("requeued 0 of 0", self._logged())
        self.assertEqual(self._pending_names(), [])
        self.assertEqual(self._sidecar_names(), ["004-d.md.claim.json"])
...
class OwnershipAwareForceTest(_WiredFixture):
    ...
    def test_an_unattributable_claim_is_refused_until_forced(self):
        ...
        self.assertEqual(metadata_path(self.claimed / "003-c.md").read_text(),
                         "{ not json",
